...
void Model::Draw(WorldTransform& worldTransform,const ViewProjection& viewProjection,
	...
	if (modelData_) {
		for (std::string name : modelData_->names) {
			...
			//Color c = material_->GetMaterialData()->color;
			//material_->GetMaterialData()->color = ObjectColor;
			// マテリアルCBufferの場所を設定
			...
			// wvp用のCBufferの場所を設定
			commandList->SetGraphicsRootConstantBufferView(3,  LightGroup::GetInstance()->GetDirectionalResource()->GetGPUVirtualAddress());
			commandList->SetGraphicsRootConstantBufferView(5, LightGroup::GetInstance()->GetPointResource()->GetGPUVirtualAddress());
			commandList->SetGraphicsRootConstantBufferView(6, LightGroup::GetInstance()->GetSpotResource()->GetGPUVirtualAddress());
			...
			//material_->GetMaterialData()->color = c;
		}
	} else {
...
Model* Model::LordModel(const std::string& filename) {
	...
	model->material_->CreateMaterialResource();
	model->GetMaterial()->GetMaterialData()->enableLighting = true;
	/*model->directionalLight_->CreateDirectionalLightResource();
	model->pointLight_->CreatePointLightResource();
	model->spotLight_->CreateSpotLightResource();*/
	return model;
...
Model::ModelData*  Model::LoadModelFile(const std::string& directoryPath, const std::string& filename)
{
	...
	for (uint32_t meshIndex = 0; meshIndex < scene->mNumMeshes; ++meshIndex) {
		...
		for (uint32_t faceIndex = 0; faceIndex < mesh->mNumFaces; ++faceIndex) {
			...
			// 四角形の処理 (四角形を2つの三角形に分割)
			else if (face.mNumIndices == 4) {
				// 四角形の1つ目の三角形 (頂点 0, 1, 2)
				modelData->object[meshName].indices.push_back(face.mIndices[0]);
				modelData->object[meshName].indices.push_back(face.mIndices[1]);
				modelData->object[meshName].indices.push_back(face.mIndices[2]);

				// 四角形の2つ目の三角形 (頂点 0, 2, 3)
				modelData->object[meshName].indices.push_back(face.mIndices[0]);
				modelData->object[meshName].indices.push_back(face.mIndices[2]);
				modelData->object[meshName].indices.push_back(face.mIndices[3]);
			}
		...
		if (material->GetTextureCount(aiTextureType_DIFFUSE) != 0) {
			...
			modelData->object[meshName].material.textureFilePath = directoryPath + "/" + filename + "/" + textureFilePath.C_Str();
			TextureManager::Load(directoryPath + "/" +filename+"/"+ textureFilePath.C_Str());
		} else
		{
			modelData->object[meshName].useTexture = false;
		}
	}
	// materialを解析する
