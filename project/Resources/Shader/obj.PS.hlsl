...
cbuffer Material : register(b0)
{
    ...
    float shininess;
    int enableEmissive; //!< エミッシブのオンオフフラグ
...
ConstantBuffer<ObjectColor> gObjectColor : register(b4);
...
PixelShaderOutput main(VertexShaderOutput input)
{
    ...
    if (enableLighting != 0)
    {
        ...
        colorMaterial.rgb * textureColor.rgb * gObjectColor.color.rgb * colorLight.rgb * cos * intensity;
        // 鏡面反射
        ...
        colorMaterial.rgb * textureColor.rgb * gObjectColor.color.rgb * gPointLight.color.rgb * cosPointLight * gPointLight.intensity * factor;
        // 鏡面反射
        ...
        colorMaterial.rgb * textureColor.rgb * gObjectColor.color.rgb * gSpotLight.color.rgb * cosSpotLight * gSpotLight.intensity * factorSpotLight * falloffFactor;
        // 鏡面反射
        ...
        output.colorPS.a = colorMaterial.a * textureColor.a * gObjectColor.color.a;
        // 映り込み追加
    ...
    {
        output.colorPS = colorMaterial * textureColor * gObjectColor.color;
    }
    // エミッシブが有効な場合、発光を加算
    if (enableEmissive != 0)
    {
        //emissiveColor.rgb = colorMaterial;
        //output.colorPS.rgb += emissiveColor.rgb * emissiveColor.a; // エミッシブカラーを加算
    }
    return output;
}
